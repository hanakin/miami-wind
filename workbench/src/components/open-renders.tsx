...
// The selectable surfaces (data-slots) per component — what the editor's target picker offers
// beyond the cva base/variants. Order is editor display order. Keep in sync with the open render.
export const editableSlots: Record<string, string[]> = {
	"dropdown-menu": [
	...
	select: ["select-trigger", "select-content", "select-label", "select-separator", "select-item"],
	...
	sheet: ["sheet-overlay", "sheet-content", "sheet-header", "sheet-title", "sheet-description"],
	...
	command: ["command-input", "command-list", "command-group", "command-separator", "command-item"],
	combobox: ["combobox-input", "combobox-content", "combobox-list", "combobox-item"],
	drawer: [
		"drawer-trigger",
		"drawer-overlay",
		"drawer-content",
...
export function slotsFor(name: string): string[] {
	return editableSlots[name] ?? [];
}
